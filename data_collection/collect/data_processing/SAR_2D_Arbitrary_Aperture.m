...
for scanIdx = 1:numScans
    ...
    for xIdx = 1:numel(imgX)
        ...
        xpos(scanIdx) = rawCollect{scanIdx}.xLoc_m;
        ypos(scanIdx) = rawCollect{scanIdx}.yLoc_m;
        ...
        mask = rangeLo<length(tmpRP);
%         z = rangeLo(1:end-trimLen);
        z = rangeLo .* mask;
        z(z==0) = [];
        test = zeros(length(mask),1);
        q = (tmpRP(z) + diffRP(z).*rangeFrac(mask==1)) .* matchVec(mask==1);
        test(mask==1) = q;
        currentPulse(:,xIdx) = test;
