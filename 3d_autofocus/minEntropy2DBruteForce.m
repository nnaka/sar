function [ focused_image, minEntropy, maxEntropy ] = minEntropy2DBruteForce( pulseSet, numIterations )
...
for iter = 1:numIterations
    ...
    for pulseIdx = 1:numPulses
        ...
        allEntropies(pulseIdx + (iter-1)*numIterations) = findEntropy(sum(scan_iter,3));
    end 
end

focused_image = sum(scan_iter, 3);
...
entropy = - sum(sum(imgNormInt.*log(imgNormInt)));
