...
sceneSizeX = 20;%6.6667;             % (m) Scene extent in x-dimension (cross-range)
...
sceneSizeY = 15;%sceneSizeX * 3/4;                      % Aspect ratio for 640 x 480 grid
...
for scanIdx = 1:numScans
    ...
    for xIdx = 1:numel(imgX)
        ...
        myImg(:,xIdx) = myImg(:,xIdx) + [(tmpRP(rangeLo(1:end-trimLen)) + diffRP(rangeLo(1:end-trimLen)).*rangeFrac(1:end-trimLen)) .* matchVec(1:end-trimLen); zeros(trimLen,1)];
    end
