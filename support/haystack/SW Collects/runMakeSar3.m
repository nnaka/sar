function [] = runMakeSar3(data)
	...
	x0=0;     % these vectors define the locations of the set of point source scatterers that we model in the scenario nid
	y0=80;
	z0=0;
	% x0=0;
	% y0=0;
	...
	%    fprintf('pass #%d\n',n);         % create a separate file to contain the signal for each line of antenna positions
	    fileName=sprintf('test%03d.wav',1);    %file name
	%    x=xCent+xStep*(-(nX-1)/2:1:(nX-1)/2);  % radar positions
	%    y=yCent*ones(size(x));
	%    z=zCent+zStep*(n-(nY-1)/2)*ones(size(x));
	    % t = data(:,1);
	    % t = t'
	    x = data(:,2);	%store IR position data for makeSAR
	    x = x';
	    y = data(:,3);
	    y = y';
	    z = data(:,4);
