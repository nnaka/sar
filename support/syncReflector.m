...
function rawCollect = syncReflector(rawCollect, positionData)
	e=0.01;			% Error
	...
	for i=1:length(rawCollect)
		...
		for j=index:length(positionData)
			...
			if (posTime-e<=rawTime && rawTime<=posTime+e) 
                ...
                rawCollect{i}.xRef_m = positionData(j,5);
				rawCollect{i}.yRef_m = positionData(j,6);
				rawCollect{i}.zRef_m = positionData(j,7);
