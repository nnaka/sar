...
function positionData = RefDataImport(fileName)
	...
	while ~feof(fid)					% iterate through all rows
		l = fgetl(fid);					
		if rowNum>50 && ~mod(rowNum-49,4)	% access row elements if row greater than 45 and even
			...
			y = [y; str2double(c(9))];
			z = [z; str2double(c(10))];
			xRef = [xRef; str2double(c(8))];
			yRef = [yRef; str2double(c(9))];
			zRef = [zRef; str2double(c(10))];
	...
	% plot3(x,y,z)
	% grid on;
	positionData = [t,x,y,z,xRef,yRef,zRef];	% concatenate data into matrix
